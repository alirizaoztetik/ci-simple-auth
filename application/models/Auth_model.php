...
class Auth_model extends MY_Model{
    ...
    function is_active($data){
        ...
        if($this->config_vars['user_is_active'] != 1){
    ...
    function is_license($data){
        ...
        if($this->config_vars['user_is_license'] != 1){
    ...
    function is_change_ip_address($data){
        if($this->config_vars['user_is_change_ip_address'] != 1){
        ...
        if($devices->num_rows() > 0){
            if($data['user_last_ip'] != $ip_address){
                if($devices->num_rows() == $this->config_vars['device_input_limit']){
                    ...
                    if($search_devices->num_rows() > 0){
                        ...
                        $code = random_string('numeric', $this->config_vars['random_code_limit']);
    ...
    function is_attempts($data){
        if($this->config_vars['user_is_attempts'] != 1){
            return $this->login($data['user_id']);
            exit;
        }
        ...
        if(!empty($attempts)){
            if(is_numeric($attempts) && $attempts <= ($this->config_vars['faulty_trial_step'] - 1)){
                $attempts++;
                ...
                if($attempts == $this->config_vars['faulty_trial_step']){
                    $attempt_date = array(
                        'user_attempt_time' => date('H:i:s', time() + ($this->config_vars['countdown_for_re_entry'] * 60)),
                        'user_updated_at' => date('Y-m-d H:i:s')
                    );
                    if($this->user_update($data->user_id, $attempt_date)){
                        ...
                        return $this->message('block', $this->config_vars['faulty_trial_step'].' '.$this->lang->line('auth_error_login_attemps_time_front').' '.$this->config_vars['countdown_for_re_entry'].' '.$this->lang->line('auth_error_login_attemps_time_end'));
                        exit;
                    }
                } else {
                    return $this->message('error', $this->lang->line('auth_error_login_attemps').' '.($this->config_vars['faulty_trial_step'] - $attempts));
                    exit;
                }
            } else {
            ...
            if($this->user_update($data->user_id, $attempt_data)){
                return $this->message('error', $this->lang->line('auth_error_login_attemps').' '.($this->config_vars['faulty_trial_step'] - 1));
                exit;
            }
        }
    }
